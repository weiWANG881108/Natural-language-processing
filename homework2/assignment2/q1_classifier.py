...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        pW = tf.Variable(tf.random_uniform((self.config.n_features, self.config.n_classes), -0.01,0.01))
        pB = tf.Variable(tf.zeros(self.config.n_classes,))
        pred = softmax(tf.matmul(self.input_placeholder, pW)+pB)
        '''
