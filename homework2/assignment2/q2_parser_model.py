...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embedding_temp, [-1, self.config.n_features*self.config.embed_size])
    ...
    def add_prediction_op(self):
        ...
        W = tf.Variable(xavier_initializer((self.config.n_features*self.config.embed_size,self.config.hidden_size)), dtype=tf.float32)
        U = tf.Variable(xavier_initializer((self.config.hidden_size, self.config.n_classes)), dtype=tf.float32)
