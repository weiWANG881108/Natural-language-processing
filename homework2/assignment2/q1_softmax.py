...
def softmax(x):
    ...
    rk = sess.run(rkNode)
    if(rk > 1):
        node2 = tf.reduce_max(x, reduction_indices=[1])
        node3 = tf.reshape(node2, [-1,1])
        node4 = x - node3
        node5= tf.cast(node4, tf.float32)
        node6= tf.exp(node5)
        node7 = tf.reduce_sum(node6,reduction_indices=[1])
        node8 = tf.reshape(node7, [-1,1])
        out = node6 / node8

    ### END YOUR CODE
    else:
        node2 = tf.reduce_max(x)
        node3 = x-node2
        node4 = tf.cast(node3, tf.float32)
        node5 = tf.exp(node4)
        node6 = tf.reduce_sum(node5)
        out = node5 / node6
