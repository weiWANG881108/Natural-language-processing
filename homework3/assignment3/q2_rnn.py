...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ### YOUR CODE HERE (~4-6 lines)
            mask = [True for item in range(len(sentence))]
            if len(sentence) > max_length:
                ret.append((sentence[0:max_length], labels[0:max_length], mask[0:max_length]))
            elif len(sentence) < max_length:
                num = max_length - len(sentence)
                sentence+=[zero_vector for i in range(num)]
                labels+=[zero_label for i in range(num)]
                mask+=[False for i in range(num)]
                print num
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        self.input_placeholder = tf.placeholder(tf.int32, shape=(None, Config.max_length, Config.n_features), name="input")
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(tf.nn.embedding_lookup(embedding, self.input_placeholder),
                                [-1, Config.max_length, Config.n_features * Config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        preds = tf.stack(preds) #(T, batch_size, n_classes)
