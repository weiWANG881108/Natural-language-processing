...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            U_z = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(), shape=[self.input_size, self.state_size], name="U_z")
            W_z = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(), shape=[self.state_size, self.state_size], name="W_z")
            b_z = tf.get_variable('b_z', [self.state_size, ], initializer=tf.constant_initializer(0), dtype=tf.float32)
            U_r = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(), shape=[self.input_size, self.state_size], name="U_r")
            W_r = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(), shape=[self.state_size, self.state_size], name="W_r")
            b_r = tf.get_variable('b_r', [self.state_size, ], initializer=tf.constant_initializer(0), dtype=tf.float32)
            U_o = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(), shape=[self.input_size, self.state_size], name="U_o")
            W_o = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(), shape=[self.state_size, self.state_size], name="W_o")
            ...
            o_temp = tf.matmul(state,W_o)
            o_t = tf.tanh(tf.matmul(inputs, U_o) + tf.multiply(r_t,o_temp) + b_o)
