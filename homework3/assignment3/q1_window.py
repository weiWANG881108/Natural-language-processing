    ...
    ### YOUR CODE HERE (5-20 lines)
        for index in range(len(sentence)):
            ...
            for i in range(window_size):
                ...
                if temp_index < 0:
                    cell_data_point = cell_data_point + start
                else: 
                    cell_data_point = cell_data_point + sentence[temp_index]
            cell_data_point = cell_data_point + sentence[index]
            for i in range(window_size):
                temp_index = index + (i+1)
                if temp_index > len(sentence)-1:
                    cell_data_point = cell_data_point + end
                else: 
                    cell_data_point = cell_data_point + sentence[temp_index]

            windowed_data.append((cell_data_point,labels[index]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        pre_embeddings = tf.Variable(self.pretrained_embeddings,trainable=False)                                                     
        embedding_temp = tf.nn.embedding_lookup(pre_embeddings,self.input_placeholder)
        embeddings = tf.reshape(embedding_temp, [-1, self.config.n_window_features*self.config.embed_size])                            
        print "*******************************"
