    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(),
                                  shape=[self.input_size, self.state_size], name="W_x")
            W_h = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(),
                                  shape=[self.state_size, self.state_size], name="W_h")
            #b = tf.get_variable(initializer=tf.zeros(self.state_size), name="b")
            b = tf.get_variable('b', [self.state_size, ], initializer=tf.constant_initializer(0), dtype=tf.float32)
